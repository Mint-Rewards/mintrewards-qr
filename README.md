...
### The card template is a stand-in

`templates/ambassador-card-background.jpg` is a **placeholder design**, generated by
`scripts/generate-card-template.mjs`, because the real designed card has not been supplied
yet. It is a complete, working template — 1080 × 1350 (Instagram's 4:5), with the branding,
title and field labels baked in and the three value slots left empty — so the whole flow
runs and is tested today. It is not the real brand asset.

To swap in the real design:

1. Flatten it to `templates/ambassador-card-background.jpg` (`pdftoppm -jpeg -r 150` for a
   PDF, or export straight to JPG/PNG from Figma/PSD/AI). It must contain everything
   static and leave the value slots empty.
2. Set `CARD_WIDTH`/`CARD_HEIGHT` in `src/lib/ambassador/card-config.ts` to its real pixel
   ...
   place.
3. Re-measure `NAME_BOX`/`UNIVERSITY_BOX`/`BATCH_BOX` against the real file, and re-measure
   the `PANEL`/`ROWS` constants in `tests/ambassador.test.ts` (which describe the template,
   not the config — see below).
...
5. Delete `scripts/generate-card-template.mjs`.
...
field, diffs the pixels, and asserts the changed region sits **below that field's printed
label and inside the panel border**. Those bounds are measured from the template image, not
imported from `card-config.ts` — asserting against the same constants that drove the render
would pass no matter how wrong the calibration was. It catches a mis-placed slot, text
overflowing the card, and a font-less deployment host.
